using MPRemoteEditor.Properties;
using ScintillaNET;
...
namespace MPRemoteEditor
{
    public partial class frmMain : Form
    {
        ...
        private MPRemoteRoutines _MPRemote = null;
        ...
        public frmMain(MPRemoteRoutines mPRemoteRoutines)
        {
            _MPRemote = mPRemoteRoutines;
        ...
        {
            this.Text = "MPRemoteEditor (" + _MPRemote.COMM_PORT + ")";
            ...
            string modules = _MPRemote.ListModules();
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                _MPRemote.PutFile(newFile, FileToAdd);
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            if (FileToExport != "")
            {
                ...
                {
                    Cursor.Current = Cursors.WaitCursor;
                    _MPRemote.GetFile(FileToExport, saveFileDialog1.FileName);
                    Cursor.Current = Cursors.Default;
                }
            }
        }
            ...
            if (selectedItem != "")
            {
                if (selectedItem.StartsWith(LBracket))
                {
                    ...
                    if (DirToDelete != "..")
                    {
                        ...
                        {
                            Cursor.Current = Cursors.WaitCursor;
                            _MPRemote.DeleteDir(FullDirToDelete);
                            Cursor.Current = Cursors.Default;
                            RefreshFileList();
                        }
                    }
                ...
                else
                {
                    ...
                    {
                        Cursor.Current = Cursors.WaitCursor;
                        _MPRemote.DeleteFile(FileToDelete);
                        Cursor.Current = Cursors.Default;
                        RefreshFileList();
                    }
                }
        ...
        private void btnMove_Click(object sender, EventArgs e)
        {
            ...
            if (FileToMove != "")
            {
                string filename = GetUserInput("Move/Rename File", "New Path and Filename:", FileToMove);
                if (filename != "")
                {
                    if (filename.IndexOf(".") > 0)
                    {
                        Cursor.Current = Cursors.WaitCursor;
                        _MPRemote.MoveFile(FileToMove, filename);
                        Cursor.Current = Cursors.Default;
                        RefreshFileList();
                    }
                    else
        ...
        private void btnMkdir_Click(object sender, EventArgs e)
        {
            ...
            if (newdir != "")
            {
                if (!newdir.Contains("."))
                {
                    ...
                    Cursor.Current = Cursors.WaitCursor;
                    _MPRemote.CreateDir(newdirfull);
                    Cursor.Current = Cursors.Default;
                    RefreshFileList();
                }
                else
        ...
        private void frmMain_DragDrop(object sender, DragEventArgs e)
        {
            ...
            foreach (string file in files)
            {
                ...
                string FileToAdd = (_CurrentPath == "") ? newFilename : _CurrentPath + "/" + newFilename;
                _MPRemote.PutFile(file, FileToAdd);
            }
            Cursor.Current = Cursors.Default;
            RefreshFileList();
        }
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            if (selectedItem != "")
            {
                if ((selectedItem != LBracket + ".." + RBracket) && (selectedItem.Substring(0, 1) != LBracket))
                {
                    ...
                    Cursor.Current = Cursors.WaitCursor;
                    _MPRemote.GetFile(ViewFile, LocalFile);
                    Cursor.Current = Cursors.Default;
            ...
            if (selectedItem != "")
            {
                ...
                else // Otherwise open the file
                {
                    ...
                    if (EditableFile(LocalFile))
                    {
                        ...
                        Cursor.Current = Cursors.WaitCursor;
                        _MPRemote.GetFile(_CurrentFile, LocalFile);
                        Cursor.Current = Cursors.Default;
                        if (File.Exists(LocalFile))
                        {
        ...
        private void RefreshFileList()
        {
            ...
            try
            {
                dir = _MPRemote.GetDir(_CurrentPath, LBracket, RBracket);
            }
            catch (Exception ex)
            {
        ...
        private bool SaveItem()
        {
            ...
            try
            {
                ...
                Cursor.Current = Cursors.WaitCursor;
                _MPRemote.PutFile(SaveFile, _CurrentFile);
                Cursor.Current = Cursors.Default;
        ...
        private void OpenREPL(string cmd)
        {
            if (ConfigurationManager.AppSettings["REPL"] == "E")
            {
                ...
                p.StartInfo.Arguments = terminalAppArgs.Replace("{PORT}", _MPRemote.COMM_PORT).Replace("{PORTNUM}", Convert.ToInt16(_MPRemote.COMM_PORT.Replace("COM", "")).ToString());
                p.Start();
            ...
            {
                scintilla1.Focus();
                TerminalForm terminal = new TerminalForm(_MPRemote.COMM_PORT, _MPRemote.BAUD_RATE, cmd, _MPRemote.DTR_ENABLED);
                terminal.ShowDialog();
            }
        }
